...
public class Aufgabe2 {
	 ...
	 * 
	 * @param a:
	 *            Parameter für Algorithmus der Pseudos-Zufallszahl-Berechnung
	 * @param b:
	 *            Parameter für Algorithmus der Pseudos-Zufallszahl-Berechnung
	 * @param m:
	 *            Parameter für Algorithmus der Pseudos-Zufallszahl-Berechnung
	 * @param seed:
	 *            Die "Saat", die als Basis für die Pseudo-Zufallszahl dient
	...
	/**
	 * Diese Methode generiert einen zufälligen PIN als String, der keine doppelten
	 * Zahlen enthält.
	 * 
	 * @param a:
	 *            s. naechsteZahl()
	...
	public static String pinGenerieren(int a, int b, int m, int seed) {
		...
		pin = "" + random;
		// Bei erfüllen der Bedingungen ist enough = true, solange Pin noch nicht alle
		// Bedingungen erfüllt ist enough = false
		...
		if (pin.length() >= 3) {
			...
			// String der Pin-Länge 4 in Char-Array "umwandeln"
			char[] pinCharacters = pin.toCharArray();

			if (pin.length() == 4) {
				enough = true;
				for (int i = 0; i < pin.length(); i++) {
					// abgleichen des derzeitigen Chars (Zahl) mit dem nächsten Char (Zahl)
	...
	public static int zaehleVieren() {
		...
		char a = '4';
		// Counter zum Zählen der Vieren in einer Zahl
		int counter = 0;
		String s = "";
